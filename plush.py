...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        while True:
            ...
            typeCtx.addFuncDef(Declaration("print_bool", [("var", "n", Type(TypeEnum.BOOL))], Type(TypeEnum.VOID)))
            typeCtx.addFuncDef(Declaration("print_int_array", [("var", "n", Type(TypeEnum.INT, 1))], Type(TypeEnum.VOID)))
            typeCtx.addFuncDef(Declaration("int_array", [], Type(TypeEnum.INT, 1)))
            verify(typeCtx, ast)
            pp_ast(ast)
            eval(ast, ValueContext())
